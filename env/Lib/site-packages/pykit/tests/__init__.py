...
def pykitcompile(source, funcname=None, cfanalyze=True):
    ...
    verify(m)
    if cfanalyze:
        for function in list(m.functions.values()):
            cfa.run(function)
            verify(function)
        verify(m)
...
def parametrize(argtuples):
    ...
    def decorator(f):
        if 'load_tests' not in f.__globals__:
            def load_tests(*args):
                return suite
            f.__globals__['load_tests'] = load_tests
            f.__globals__['__suite'] = unittest.TestSuite()
        suite = f.__globals__['__suite']
