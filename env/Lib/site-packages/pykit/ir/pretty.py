...
def fmod(mod):
    gs, fs = list(mod.globals.values()), list(mod.functions.values())
    return njoin([njoin(list(map(pretty, gs))), "", njoin(list(map(pretty, fs)))])

def ffunc(f):
    restype = ftype(f.type.restype)
    types, names = list(map(ftype, f.type.argtypes)), list(map(prefix, f.argnames))
    args = ajoin(list(map(sjoin, list(zip(types, names)))))
    header = sjoin(["function", restype, f.name + parens(args)])
    return njoin([header + " {", njoin(list(map(fblock, f.blocks))), "}"])

def farg(func_arg):
    return "%" + func_arg.result

def fblock(block):
    body = njoin(list(map(compose(indent, fop), block)))
...
def fop(op):
    body = "%s(%s)" % (op.opcode, ajoin(list(map(_farg, op.args))))
