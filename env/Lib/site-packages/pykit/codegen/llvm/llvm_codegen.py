...
i1, i16, i32, i64 = list(map(Type.int, [1, 16, 32, 64]))
...
class Translator(object):
    ...
    def op_call_math(self, op, name, args):
        ...
        lfunc = self.lmod.get_or_insert_function(
            lfunc_type, 'pykit.math.%s.%s' % (list(map(str, argtypes)), name.lower()))
...
def update_phis(phis, valuemap, argloader):
    ...
    for phi in phis:
        ...
        llvm_blocks = list(map(argloader.load_op, phi.args[0]))
        llvm_values = list(map(argloader.load_op, phi.args[1]))
